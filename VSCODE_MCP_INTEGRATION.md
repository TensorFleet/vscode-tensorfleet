...
Simulation movement preflight tools `vacuum_check_navigation_readiness` and `vacuum_check_clean_area_readiness` are read-only. They return structured readiness data for the current simulation movement-start tools, including `ready`, `status`, `blockingReasons`, `warnings`, `requiredInputs`, `capabilities`, and `snapshotEvidence`, but they do not dispatch navigation, go-to, Clean Area, room cleaning, or zone cleaning. `vacuum_get_supported_actions` summarizes current read tools, active mission actions, the callable simulation `vacuum_start_navigation` and `vacuum_start_clean_area` writes when their normalized capabilities are supported, and still-deferred movement starts.

`vacuum_start_navigation` is simulation-backend-only. It requires a target with numeric `x`, `y`, and `theta`, treats the target as a map-frame product target, runs runtime/source, map, pose/localization, active mission, and `start_navigation` capability gates before dispatch, and sends only the normalized product command through `POST /vms/self/tensorfleet/api/v1/vacuum/command`.

`vacuum_start_clean_area` is simulation-backend-only. It accepts only a simple rectangle area with numeric `x`, `y`, positive `width`, positive `height`, optional `frameId: "map"`, and optional request-context `label`; it runs runtime/source, map, pose/localization, active mission, and `start_coverage` capability gates before dispatching the normalized `start_coverage` command through `POST /vms/self/tensorfleet/api/v1/vacuum/command`.
