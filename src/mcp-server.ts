...
type VacuumBackend = "simulation" | "real_vacuum";
type VacuumBackendAdapter = "turtlebot4_nav2" | "valetudo";
type VacuumConfigSource = "argument" | "process-env" | "missing";

const VACUUM_DISCOVERY_TOOL = "vacuum_get_supported_actions";
const VACUUM_PLUGIN_TOOL = "tensorfleet-vacuum";
const VACUUM_PLUGIN_ACTION = "get-supported-actions";
...
function resolveVacuumBackend(args: Record<string, unknown>):
  | {
      ok: true;
      input: string;
      source: VacuumConfigSource;
      backend: VacuumBackend;
      backendAdapter: VacuumBackendAdapter;
    }
  | {
      ok: false;
      input?: string;
  ...
  const argumentBackend = pickString(args, "backend");
  const envBackend = process.env.TENSORFLEET_VACUUM_BACKEND;
  const selected = argumentBackend ?? envBackend;
  const source: VacuumConfigSource = argumentBackend ? "argument" : envBackend ? "process-env" : "missing";

  if (!selected) {
    return {
      ok: false,
      source,
      message: "No TensorFleet vacuum backend is selected. Pass backend=simulation or backend=real_vacuum explicitly.",
    };
  }

  if (["simulation", "turtlebot4_nav2", "turtlebot4-nav2"].includes(selected)) {
    return {
      ok: true,
      input: selected,
      source,
  ...
  if (["real_vacuum", "real-vacuum", "valetudo"].includes(selected)) {
    return {
      ok: true,
      input: selected,
      source,
  ...
  return {
    ...
    input: selected,
    source,
    message: `Unsupported TensorFleet vacuum backend: ${selected}.`,
  };
}

function settingStatus(value: string | undefined): { available: boolean; source: VacuumConfigSource } {
  return {
    available: value != null && value.trim().length > 0,
    source: value != null && value.trim().length > 0 ? "process-env" : "missing",
  };
}

function buildVacuumMcpDiscoveryResponse(args: Record<string, unknown>) {
  const timestamp = new Date().toISOString();
  const backend = resolveVacuumBackend(args);
  const auth = settingStatus(process.env.TENSORFLEET_JWT);
  const vmManagerUrl = settingStatus(process.env.TENSORFLEET_VM_MANAGER_URL);
  const runtimeUrl = settingStatus(process.env.TENSORFLEET_VALETUDO_RUNTIME_URL);

  if (!backend.ok) {
    return {
      success: false,
      action: VACUUM_PLUGIN_ACTION,
      mcpTool: VACUUM_DISCOVERY_TOOL,
      ...
      surfaces: {
        openclawPlugin: { tool: VACUUM_PLUGIN_TOOL, action: VACUUM_PLUGIN_ACTION },
        mcp: { tool: VACUUM_DISCOVERY_TOOL },
      },
      actions: {
        readOnlyCallableTools: [{ tool: VACUUM_DISCOVERY_TOOL }],
      ...
      movementBlockers: [backend.message],
    };
  }

  const routeMode = pickString(args, "routeMode") === "direct" ? "direct" : "vm-manager";
  const blockers: string[] = [];
  if (routeMode === "vm-manager") {
  ...
  return {
    ...
    backendSelection: {
      ...
      normalizedBackendAdapter: backend.backendAdapter,
    },
    timestamp,
    surfaces: {
      openclawPlugin: {
        tool: VACUUM_PLUGIN_TOOL,
        action: VACUUM_PLUGIN_ACTION,
        note: "Preferred TensorFleet agent surface when the OpenClaw plugin capability is loaded.",
      },
      mcp: {
        tool: VACUUM_DISCOVERY_TOOL,
        note: "Discovery-only MCP compatibility surface; no snapshot, command, raw ROS, or hardware-control MCP tools are exposed in this rollout step.",
      },
    },
    runtime: {
      routeMode,
    ...
    actions: {
      readOnlyCallableTools: [{ tool: VACUUM_DISCOVERY_TOOL }],
      pluginEquivalentReadOnlyAction: { tool: VACUUM_PLUGIN_TOOL, action: VACUUM_PLUGIN_ACTION },
      movementStartCallableTools: [],
      missionControlCallableTools: [],
      stateChangingCallableTools: [],
      writeCapableButGatedActions: [],
      deferredActions: [
        "get-snapshot",
        "get-map-summary",
        "get-mission-state",
        ...
        action,
        callable: false,
        reason: "Not exposed through the OpenClaw-managed MCP server in Step 1.5.",
      })),
      unsupportedActions: backend.backend === "simulation"
        ? [
            {
              action: "send-command",
              callable: false,
              reason: "Simulation command dispatch is not exposed as an OpenClaw MCP tool in Step 1.5.",
            },
          ]
        : [],
    },
    ...
    movementBlockers: [
      ...blockers,
      "Step 1.5 is discovery/readiness only and does not expose movement-start MCP tools.",
    ],
  };
}
...
class TensorFleetMCPServer {
  ...
  private setupTools() {
    // Discovery-only TensorFleet vacuum tool for OpenClaw-managed MCP.
    this.tools.set(VACUUM_DISCOVERY_TOOL, {
      execute: async (args) => ({
        content: [
          {
            type: "text",
            text: JSON.stringify(buildVacuumMcpDiscoveryResponse(args), null, 2),
          },
        ],
      }),
    });
  ...
  private setupRequestHandlers() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          {
            name: VACUUM_DISCOVERY_TOOL,
            description: "Discover TensorFleet vacuum agent surfaces, selected backend, config/auth readiness, deferred actions, and movement availability without contacting ROS, Valetudo, or hardware",
            inputSchema: {
              type: "object",
              ...
              properties: {
                backend: {
                  type: "string",
                  enum: ["simulation", "real_vacuum", "turtlebot4_nav2", "valetudo"],
                  description: "Vacuum backend. Pass simulation for the TurtleBot4/Nav2 VM backend, or real_vacuum for the Valetudo-backed real-vacuum integration.",
                },
                routeMode: {
                  type: "string",
                  enum: ["vm-manager", "direct"],
                  description: "Runtime route to check for readiness. Defaults to vm-manager.",
                },
              },
            },
          },
