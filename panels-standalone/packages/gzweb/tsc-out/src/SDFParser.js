...
import { FuelServer, createFuelUri } from "./FuelServer";
import { Color } from "./Color";
import { Inertia } from "./Inertia";
import { Material } from "./Material";
import { PBRMaterial } from "./PBRMaterial";
import { Pose } from "./Pose";
import System, { Body, BoxZone, Emitter, Life, Position, Radius, Rate, Scale, Span, SpriteRenderer, VectorVelocity,
// @ts-ignore
 } from "three-nebula";
class PendingMesh {
    constructor() {
...
export class SDFParser {
    ...
    parse3DVector(vectorInput) {
        let vector3D = new THREE.Vector3();
        var values = [];
    ...
    createMaterial(srcMaterial) {
        ...
        if (srcMaterial.pbr && this.enablePBR) {
            ...
            if (srcMaterial.pbr.metal) {
                ...
                material.pbr.ambientOcclusionMap =
                    srcMaterial.pbr.metal.ambient_occlusion_map;
            }
                ...
                material.pbr.ambientOcclusionMap =
                    srcMaterial.pbr.specular.ambient_occlusion_map;
                ...
                material.pbr.ambientOcclusionMap =
                    srcMaterial.pbr.ambient_occlusion_map;
            }
        }
        ...
        if (material.pbr && this.enablePBR) {
            ...
            if (material.pbr.emissiveMap) {
                let emissiveMap = "";
                let emissiveMapName = material.pbr.emissiveMap
                    .split("/")
                    .pop();
            ...
            if (material.pbr.roughnessMap) {
                let roughnessMap = "";
                let roughnessMapName = material.pbr.roughnessMap
                    .split("/")
                    .pop();
            ...
            if (material.pbr.metalnessMap) {
                let metalnessMap = "";
                let metalnessMapName = material.pbr.metalnessMap
                    .split("/")
                    .pop();
    ...
    createGeom(geom, mat, parent, options) {
        ...
        if (geom.box) {
            ...
            this.scene.loadHeightmap(geom.heightmap.heights, geom.heightmap.size.x, geom.heightmap.size.y, geom.heightmap.width, geom.heightmap.height, new THREE.Vector3(geom.heightmap.origin.x, geom.heightmap.origin.y, geom.heightmap.origin.z), geom.heightmap.texture, geom.heightmap.blend, parent);
        }
        if (obj) {
            if (material) {
    ...
    createVisual(visual, options) {
        let visualObj = new THREE.Object3D();
    ...
    createSensor(sensor, options) {
        let sensorObj = new THREE.Object3D();
    ...
    loadSDF(sdfName, callback) {
        ...
        this.fileFromUrl(filename, function (sdf) {
            if (!sdf) {
                console.error("Error: Failed to get the SDF file (" +
                    filename +
    ...
    spawnModelFromSDF(sdfObj, options) {
        // create the model
        let modelObj = new THREE.Object3D();
    ...
    createLink(link, options) {
        ...
        let visualObj;
        let sensorObj;
        let linkObj = new THREE.Object3D();
     ...
     */
    createCapsuleSDF(translation, euler) {
        var geomSDF = "<capsule>" +
            "<radius>0.5</radius>" +
            "<length>1.0</length>" +
            "</capsule>";
