...
function getHeadingRadians(state: Partial<DroneState>): number {
    const hdg = state.global_position_int?.hdg;
    if (typeof hdg === 'number' && hdg !== 65535) return (hdg / 100) * (Math.PI / 180);
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
  <path d="M16 4 L22 28 L16 22 L10 28 Z" fill="rgba(255,0,0,0.5)"/>
</svg>`
    );

function makeDroneStyles(rotationRad: number): Style[] {
    const rot = rotationRad - Math.PI / 2;
...
export const DroneMap: React.FC<Props> = ({ model, follow = true, className }) => {
    ...
    const currentCoordRef = useRef<[number, number]>(fromLonLat([0, 0]));
    const targetCoordRef = useRef<[number, number]>(fromLonLat([0, 0]));
    ...
    useEffect(() => {
        ...
        const unsubscribe = model.onUpdate((state) => {
            ...
            const headingRad = getHeadingRadians(state);
