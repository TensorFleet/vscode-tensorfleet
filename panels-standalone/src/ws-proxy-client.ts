...
export class ProxyWebSocketClient {
    ...
    connect(): void {
        ...
        try {
            ...
            this.ws = this.config.subprotocols?.length
                ? new WebSocket(this.config.proxyUrl, this.config.subprotocols)
                : new WebSocket(this.config.proxyUrl);
    ...
    private sendLogin(): void {
        ...
        console.log('[ProxyWsClient] Sending login for node:', this.config.nodeId, 'port:', this.config.targetPort);
    ...
    private scheduleReconnect(): void {
        ...
        console.log(`[ProxyWsClient] Scheduling reconnect attempt ${this.reconnectAttempts} in ${delay}ms`);
...
export function createProxyWebSocket(
    ...
    const ws = {
        ...
        get readyState() {
            return proxyClient.isConnected() ? WebSocket.OPEN : WebSocket.CONNECTING;
        },
